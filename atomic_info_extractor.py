...
def get_six_neighbor_atoms(neighbor_element_type,atomic_number):
    ...
    OF=calculate_octahedral_factor(center_coord, oxygen_coords)
...
def calculate_octahedral_factor(center_atom, oxygen_atoms):
    """
...
for i,v in enumerate(element_row):
        ...
        #octa_data_i=0
    octa_data_i,OF_out,ADM_out,DI_out=get_six_neighbor_atoms(ele_neighbor,v[1])
    output_data.append(f"{v[1]}-{str(v[0])[:-1]}\t{octa_data_i}\t{OF_out}\t{ADM_out}\t{DI_out}")

f_out=open(out_file,"w+")
f_out.writelines("#atom_name\t八面体体积\t八面体因子OF\t平均角度畸变\t畸变因子DI\n")
