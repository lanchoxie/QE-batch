...
time_schedule=[["LiNiO2_331_NCMT_4131-exhange_file-gamma","relax"],["LiNiO2_331_NCMT_4131-exhange_file-221kp","relax"],["LiNiO2_331_NCMT_4131-exhange_file-beta-0.4","relax"],["LiNiO2_331_NCMT_4131-exhange_file-beta-0.5","relax"],["LiNiO2_331_NCMT_4131-exhange_file-beta-0.6","relax"]]        #[[dir1,mode1],[dir2,mode2],...]
    ...
    if file_states=="None":
        ...
        for i in range(len(read_datas)):
            if (read_datas[i][-1].find("No Calculation")!=-1)|(read_datas[i][-1].find("not converged")!=-1)|(read_datas[i][-1].find("out of timing")!=-1)|(read_datas[i][-1].find("ERROR")!=-1):
                #print("python %s/continue_cal.py %s relax"%(QE_dir,read_datas[i][0]))
                if debug==0:
